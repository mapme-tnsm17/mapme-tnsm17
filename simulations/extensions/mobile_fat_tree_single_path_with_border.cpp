...
#include "mobile_fat_tree_single_path_with_border.hpp"
...
MobileFatTreeTopologySinglePathWithBorder::MobileFatTreeTopologySinglePathWithBorder(NodeContainer & Leaves, Options & options, bool has_meshed_leaves)
  ...
  , m_options(options)
{
    ...
    for (uint32_t aggr = 1; aggr <=4; aggr++) {
     ...
     //add horizontal links between aggregation nodes
     addLink(Get(1), Get(4), bw_aggr);
     addLink(Get(2), Get(3), bw_aggr);
...
Ptr<Node>
MobileFatTreeTopologySinglePathWithBorder::GetRoot()
{
    return Get(0);
}

void
MobileFatTreeTopologySinglePathWithBorder::GetRootNodes(NodeContainer & n)
{
    n.Add(GetRoot());
}

void
MobileFatTreeTopologySinglePathWithBorder::LeafSetPosition(Ptr<Node> leaf, uint32_t x, uint32_t y)
{
...
void
MobileFatTreeTopologySinglePathWithBorder::addLink(Ptr<Node> n1, Ptr<Node> n2, std::string bandwidth)
{
    NodeContainer nodes;
    nodes.Add(n1);
    nodes.Add(n2);
...
void
MobileFatTreeTopologySinglePathWithBorder::addx2Link(Ptr<Node> n1, Ptr<Node> n2, std::string bandwidth)
{
    NodeContainer nodes;
    nodes.Add(n1);
    nodes.Add(n2);
...
void MobileFatTreeTopologySinglePathWithBorder::try_to_add_x2_link(Ptr<Node> n1, Ptr<Node> n2, std::string bandwidth){
